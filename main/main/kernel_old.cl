...
__constant int MAXITER = 2;
//模拟退火算法的参数设置
__constant int T0 = 20;		//模拟退火算法中的初始温度
__constant int NMAX = 200;		//模拟退火算法中一次降温的采样数

__constant float DEC_SPEED = 0.05;

//粒子群算法的参数设置
#define ciParticleNum  100	//粒子数
__constant int ciMax = 10;			//采样次数

__constant float dC1 = 2;					//权重c1
__constant float dC2 = 2;					//权重c2

__constant float cdVmax = 5;				//单次位移最大值
__constant float dW = 1;
...
__constant float3 f3EyePos = (float3)(0, 1, 7.0);
						...
						)
{
	...
	if((idx>= *splitNodeArrayBeg)&&(idx<= *splitNodeArrayEnd) && (splitNodeArray[idx].end - splitNodeArray[idx].beg > 64) && (splitNodeArray[idx].beg != -1) && (splitNodeArray[idx].end != -1))
	{
		...
		__local float adY[ciParticleNum];
		__local float adV[ciParticleNum];
		__local float adBest[ciParticleNum];
...
__kernel void RayTrace(__global const struct SplitNode* spSplitNodeArray, __global const int* iWinWidth, __global const int* iWinHeight, __global unsigned char* pcResPB, __global const struct TriangleInfo* TriangleInfoArray, __global const struct TriangleCandidateSplitPlane* input, __global const int*  length, __global const double* xPos, __global const double* yPos)
{
	...
	int i = get_global_id(1);
	float3 f3PixPos = (float3)((idx - (*iWinWidth)/2.0  )/256.0 + f3EyePos.x, ( i - (*iWinHeight)/2.0  )/256.0 + f3EyePos.y, 4.5);
